...
def objective(trial, config, trainloader, testloader, device):
    sigma_list = [
        trial.suggest_float(f'sigma_layer1', 0.05, 10.0),
        trial.suggest_float(f'sigma_layer2', 0.05, 10.0),
        trial.suggest_float(f'sigma_layer3', 0.05, 10.0),
        trial.suggest_float(f'sigma_layer4', 0.05, 10.0),
    ]
    ...
    sigma_str = ", ".join([f"σ{i+1}={sigma_list[i]:.3f}" for i in range(len(sigma_list))])
    print(f"Trial {trial.number}: Val Acc: {best_val_acc:.2f}%, Val Loss: {best_val_loss:.4f}, Sigma: [{sigma_str}]")
...
def main():
    ...
    print("="*60)
    print(f"Optuna Hyperparameter Search for ViT-GLU")
    print("="*60)
    print(f"Device: {device}")
    print(f"Activation: {config['activation_name']}")
    print(f"Number of trials: {config['n_trials']}")
    print(f"Epochs per trial: {config['num_epochs']}")
    print(f"Search range: σ ∈ [0.05, 10.0] for each layer")
    print("="*60)
    ...
    study.optimize(
        lambda trial: objective(trial, config, trainloader, testloader, device),
    ...
    result_filename = f'optuna_best_{config["activation_name"]}.json'
    with open(result_filename, 'w') as f:
        json.dump(best_result, f, indent=4)
    print(f"\nBest results saved to {result_filename}")
    ...
    csv_filename = f'optuna_trials_{config["activation_name"]}.csv'
    df.to_csv(csv_filename, index=False)
    print(f"All trials saved to {csv_filename}")
